...
BayesFactor::anovaBF(h.value ~ strides * drops, data = dat.long)

m0 = aov(h.value ~ strides * drops * data.version * h.target, data = dat.long)
summary(m0)

m0 = lme4::lmer(h.value ~ strides * drops * data.version + (1 | id), data = dat.long)
summary(m0)

t.test(dat.long$h.value)
...
# Absolute value if wanted
# dat.long$h.value = abs(dat.long$h.value)

# Mean and 95% credible interval
ci95 = dat.long |>
  dplyr::group_by(data.version, h.target, strides) |>
  dplyr::summarise(tmp = list(bayestestR::ci(h.value, ci = 0.95)),
                   h.mean = mean(h.value, na.rm = TRUE),
                   .groups = "drop") |>
...
ggplot(dat.long, aes(x = h.value, fill = data.version)) +
  ...
  # Mean lines
  geom_vline(data = ci95,
             aes(xintercept = h.mean, color = data.version),
             linewidth = 0.8, linetype = "solid", show.legend = FALSE) +
  # 95% credible interval bounds (dashed)
  geom_vline(data = ci95,
             aes(xintercept = CI_low, color = data.version),
             linewidth = 0.8, linetype = "dashed", show.legend = FALSE) +
  geom_vline(data = ci95,
             aes(xintercept = CI_high, color = data.version),
             linewidth = 0.8, linetype = "dashed", show.legend = FALSE) +
  # More beautifying
  scale_fill_manual(values = c("random.diff" = scales::alpha("blue", 0.1),
                               "contiguous.diff" = scales::alpha("red", 0.1)),
                    name = "Data Version",
                    labels = c("Random", "Contiguous")) +
  scale_color_manual(values = c("random.diff" = "blue",
                                "contiguous.diff" = "red")) +
