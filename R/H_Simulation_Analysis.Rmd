...
dat.long$strides.new.length = factor(dat.long$strides.new.length)
...
dat.long = dat.long[!is.nan(strides)] # Remove skipped trials
...
ci95 = dat.long |>
  dplyr::group_by(data.version, strides, drops) |>
  dplyr::summarise(h.mean = mean(h.value.abs, na.rm = TRUE),
                   h.median = median(h.value.abs, na.rm = TRUE),
                   .groups = "drop")
...
dat = dat[!is.nan(strides)] # Remove skipped trials
...
cool = dat.long[!(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  dplyr::group_by(strides, drops, data.version) %>%
  rstatix::wilcox_test(h.value ~ 1, mu = 0) %>%
  rstatix::add_significance()

cool.abs = dat.long[!(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  dplyr::group_by(strides, drops, data.version) %>%
  rstatix::wilcox_test(h.value.abs ~ 1, mu = 0) %>%
  rstatix::add_significance()

# Between H values themselves
cool = dat.long[(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  dplyr::group_by(strides, drops) %>%
  rstatix::wilcox_test(h.value ~ data.version, paired = TRUE) %>%
  rstatix::add_significance()

cool.abs = dat.long[(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  dplyr::group_by(strides, drops) %>%
  rstatix::wilcox_test(h.value.abs ~ data.version, paired = TRUE) %>%
  rstatix::add_significance()
...
results = dat.long[!(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  group_by(strides, drops, data.version) %>%
  group_split() %>%
  map_df(function(df) {
    y2 = df$h.value
    mu = rep(0, length(y2))
    ...
    tibble(strides = unique(df$strides),
           drops = unique(df$drops),
           variable = unique(df$data.version),
           bayes_factor = res$BF10)
  })

results.abs = dat.long[!(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  group_by(strides, drops, data.version) %>%
  group_split() %>%
  map_df(function(df) {
    y2 = df$h.value.abs
...
results = dat.long[(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  group_by(strides, drops) %>%
  group_split() %>%
  map_df(function(df) {
    versions =
      intersect( c('original', 'random', 'contiguous'), unique(df$data.version))
    
    # all pairwise combinations of versions
    pairs = expand.grid( v1 = versions, v2 = versions, stringsAsFactors = FALSE ) %>%
      dplyr::filter(v1 < v2)
    ...
    map_df(seq_len(nrow(pairs)), function(i) {
      ...
      y1 = df$h.value[df$data.version == v1]
      y2 = df$h.value[df$data.version == v2]
      
      res = DFBA::dfba_wilcoxon( Y1 = y1, Y2 = y2)
...
results.abs = dat.long[(dat.long$data.version %in% c('original', 'random', 'contiguous')), ] %>%
  ...
  map_df(function(df) {
    ...
    map_df(seq_len(nrow(pairs)), function(i) {
      ...
      y1 = df$h.value.abs[df$data.version == v1]
      y2 = df$h.value.abs[df$data.version == v2]
